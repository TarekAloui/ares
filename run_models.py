...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description="Ares: A framework for quantifying the resilience of deep neural networks",
    ...
    parser.add_argument(
        "-to", "--train_once", action="store_true", help="Train the model."
    )

    parser.add_argument(
        "-sw", "--save_weights", action="store_true", help="Save weights to cache."
    )
    parser.add_argument(
        "-lw",
        "--load_weights",
        action="store_true",
        help="Load saved weights from cache.",
    )
    parser.add_argument(
        "-ld_rt",
        "--load_pruned_value",
    ...
    parser.add_argument(
        "-l2", "--l2", default="0.0", type=float, help="Set l2 reg penalty."
    )
    parser.add_argument(
        "-dr_rt",
        "--dropout_rate",
        default="0.35",
        type=float,
        help="Dropout rate for appropriate layers.",
    )
    parser.add_argument(
        "-eps", "--epochs", default=15, type=int, help="Num of training epochs."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Plot each layer weight distributions.",
    )
    parser.add_argument(
        "-c",
        "--configuration",
    ...
    parser.add_argument(
        "-cache", "--cache", type=str, default=None, help="Specify a cache dir."
    )
    parser.add_argument(
        "-results", "--results", type=str, default=None, help="Specify results dir."
    )
    parser.add_argument(
        "-seed",
        "--seed",
